import { snippetFrom } from "../utils/snippet.js";
...
export function createTriage(payload) {
  ...
  const assigned_team = routeTeam(category);
...
export function normalizeEmailForInsert(raw) {
  return {
    ...raw,
    snippet: raw.snippet || snippetFrom(raw.body),
    received_at: raw.received_at ? new Date(raw.received_at) : new Date(),
    triage: raw.triage
      ? {
          ...raw.triage,
          last_updated_at: raw.triage.last_updated_at ? new Date(raw.triage.last_updated_at) : new Date(),
        }
      : undefined,
  };
}
